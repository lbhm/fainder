    ...
    for i in {1..5}; do
        ...
        for k in {50..250..10} {300..1000..50}; do
            cluster-histograms \
                ...
                -o data/"$dataset"/clusterings/runtime_benchmark/k"$k"-b50000.zst \
                -a kmeans \
                -c "$k" "$k" \
                -b 50000 \
                -t quantile \
                --alpha 1 \
                --seed 42 \
                --log-file logs/runtime_benchmark/indexing/"$dataset"-clustering-k"$k"-"$i".log \
                --log-level "$log_level"
            create-index \
                -i data/"$dataset"/clusterings/runtime_benchmark/k"$k"-b50000.zst \
                -m rebinning \
                -o data/"$dataset"/indices/runtime_benchmark \
                --index-file "rebinning-k$k.zst" \
                --log-file logs/runtime_benchmark/indexing/"$dataset"-rebinning-k"$k"-"$i".log \
                --log-level "$log_level"
            create-index \
                -i data/"$dataset"/clusterings/runtime_benchmark/k"$k"-b50000.zst \
                -m conversion \
                ...
                --log-file logs/runtime_benchmark/indexing/"$dataset"-conversion-k"$k"-"$i".log \
                --log-level "$log_level"
        done
        ...
        for budget in 1000 5000 10000 50000 100000; do
            cluster-histograms \
                -i data/"$dataset"/histograms.zst \
                -o data/"$dataset"/clusterings/runtime_benchmark/k"$k"-b"$budget".zst \
                -a kmeans \
                -c "$k" "$k" \
                -b "$budget" \
                -t quantile \
                --alpha 1 \
                --seed 42 \
                --log-file logs/runtime_benchmark/indexing/"$dataset"-clustering-b"$budget"-"$i".log \
                --log-level "$log_level"
            create-index \
                -i data/"$dataset"/clusterings/runtime_benchmark/k"$k"-b"$budget".zst \
                -m rebinning \
                -o data/"$dataset"/indices/runtime_benchmark \
                --index-file "rebinning-b$budget.zst" \
                --log-file logs/runtime_benchmark/indexing/"$dataset"-rebinning-b"$budget"-"$i".log \
                --log-level "$log_level"
            create-index \
                -i data/"$dataset"/clusterings/runtime_benchmark/k"$k"-b"$budget".zst \
                -m conversion \
                -o data/"$dataset"/indices/runtime_benchmark \
                --index-file "conversion-b$budget.zst" \
                --log-file logs/runtime_benchmark/indexing/"$dataset"-conversion-b"$budget"-"$i".log \
                --log-level "$log_level"
        done
    done
done
