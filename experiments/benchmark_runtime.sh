...
for dataset in "sportstables" "open_data_usa" "gittables"; do
    ...
    cluster-histograms -i data/"$dataset"/histograms.zst -o data/"$dataset"/clusterings/runtime_benchmark.zst -a kmeans -c 2 150 -b 50000 -t quantile --alpha 1 --seed 42 --log-file logs/runtime_benchmark/indexing/"$dataset"-clustering.log
    create-index -i data/"$dataset"/clusterings/runtime_benchmark.zst -m rebinning -o data/"$dataset"/indices/runtime_benchmark --index-file rebinning.zst --log-file logs/runtime_benchmark/indexing/"$dataset"-rebinning.log
    ...
    for i in {1..5}; do
        ...
        ### Additional Binsort scaling experiments ###
