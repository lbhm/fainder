...
for dataset in "open_data_usa"; do # "sportstables" "gittables"
    ...
    for k in "${k_values[@]}"; do
        ...
        cluster-histograms \
            -i data/"$dataset"/histograms.zst \
            -o data/"$dataset"/clusterings/microbenchmarks/k"$k"-b50000.zst \
            -a kmeans \
            -c "$k" "$k" \
            -b 50000 \
            -t quantile \
            --alpha 1 \
            --seed 42 \
            --log-file logs/microbenchmarks/indexing/"$dataset"-clustering-k"$k".log \
            --log-level "$log_level"
        create-index \
            -i data/"$dataset"/clusterings/microbenchmarks/k"$k"-b50000.zst \
            -m rebinning \
            -o data/"$dataset"/indices/microbenchmarks \
            --index-file "rebinning-k$k.zst" \
            --log-file logs/microbenchmarks/indexing/"$dataset"-rebinning-k"$k".log \
            --log-level "$log_level"
        create-index \
            -i data/"$dataset"/clusterings/microbenchmarks/k"$k"-b50000.zst \
            -m conversion \
            -o data/"$dataset"/indices/microbenchmarks \
            --index-file "conversion-k$k.zst" \
            --log-file logs/microbenchmarks/indexing/"$dataset"-conversion-k"$k".log \
            --log-level "$log_level"
        ...
        for i in {1..5}; do
            ...
            run-queries \
                ...
                -m recall \
                --log-file logs/microbenchmarks/runtime/"$dataset"-rebinning-k"$k"-single-"$i".log \
                --log-level "$log_level"
            run-queries \
                -i data/"$dataset"/indices/microbenchmarks/rebinning-k"$k".zst \
                -t index \
                ...
                -m recall \
                --suppress-results \
                --log-file logs/microbenchmarks/runtime/"$dataset"-rebinning-k"$k"-single_suppressed-"$i".log \
                --log-level "$log_level"
        done
    ...
    for b in 100 500 1000 5000 10000 50000 100000 500000 1000000; do
        ...
        cluster-histograms \
            -i data/"$dataset"/histograms.zst \
            -o data/"$dataset"/clusterings/microbenchmarks/k100-b"$b".zst \
            -a kmeans \
            -c 100 100 \
            -b "$b" \
            -t quantile \
            --alpha 1 \
            --seed 42 \
            --log-file logs/microbenchmarks/indexing/"$dataset"-clustering-b"$b".log \
            --log-level "$log_level"
        create-index \
            -i data/"$dataset"/clusterings/microbenchmarks/k100-b"$b".zst \
            -m rebinning \
            -o data/"$dataset"/indices/microbenchmarks \
            --index-file "rebinning-b$b.zst" \
            --log-file logs/microbenchmarks/indexing/"$dataset"-rebinning-b"$b".log \
            --log-level "$log_level"
        create-index \
            -i data/"$dataset"/clusterings/microbenchmarks/k100-b"$b".zst \
            -m conversion \
            -o data/"$dataset"/indices/microbenchmarks \
            --index-file "conversion-b$b.zst" \
            --log-file logs/microbenchmarks/indexing/"$dataset"-conversion-b"$b".log \
            --log-level "$log_level"
        ...
        for i in {1..5}; do
            ...
            run-queries \
                ...
                -q data/"$dataset"/queries/microbenchmarks.zst \
                -m recall \
                --log-file logs/microbenchmarks/runtime/"$dataset"-rebinning-b"$b"-single-"$i".log \
                --log-level "$log_level"
            run-queries \
                -i data/"$dataset"/indices/microbenchmarks/rebinning-b"$b".zst \
                -t index \
                ...
                -m recall \
                --suppress-results \
                --log-file logs/microbenchmarks/runtime/"$dataset"-rebinning-b"$b"-single_suppressed-"$i".log \
                --log-level "$log_level"
        done
