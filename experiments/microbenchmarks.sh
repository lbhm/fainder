...
for k in 1 {10..50..10} {100..1000..50}; do
    ...
    for i in {1..5}; do
        ...
        run-queries -i data/"$dataset"/indices/microbenchmarks/rebinning-k"$k".zst -t index -q data/"$dataset"/queries/test-all.zst -m recall --log-file logs/microbenchmarks/runtime/"$dataset"-rebinning-k"$k"-single-"$i".log
        run-queries -i data/"$dataset"/indices/microbenchmarks/rebinning-k"$k".zst -t index -q data/"$dataset"/queries/test-all.zst -m recall --suppress-results --log-file logs/microbenchmarks/runtime/"$dataset"-rebinning-k"$k"-single_suppressed-"$i".log
    done
    ...
    python experiments/compute_fainder_results.py \
        ...
        -t data/"$dataset"/results/ground_truth-test-all.zst \
...
for b in 100 500 1000 5000 10000 50000 100000 500000 1000000; do
    ...
    for i in {1..5}; do
        # Measure runtime
        # NOTE: We only execute the queries for rebinning here, since conversion is equally fast
        run-queries -i data/"$dataset"/indices/microbenchmarks/rebinning-b"$b".zst -t index -q data/"$dataset"/queries/test-all.zst -m recall --log-file logs/microbenchmarks/runtime/"$dataset"-rebinning-b"$b"-single-"$i".log
        run-queries -i data/"$dataset"/indices/microbenchmarks/rebinning-b"$b".zst -t index -q data/"$dataset"/queries/test-all.zst -m recall --suppress-results --log-file logs/microbenchmarks/runtime/"$dataset"-rebinning-b"$b"-single_suppressed-"$i".log
    done
